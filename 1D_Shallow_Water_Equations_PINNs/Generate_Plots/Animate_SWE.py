...
def Animate_PDE_Losses(frames_per_second=10):
    ...
    number_of_frames = len(symbolic_function_u_over_training)
    ...
    training_steps = range(Hyper_Parameter_Dictionary["training_steps"] + 1)
    ...
    def update_network_output_u(frame, network_output_u_axis):
        step = frame * Hyper_Parameter_Dictionary["training_steps"] / 1000.0
        ...
        if Hyper_Parameter_Dictionary["non_dimensionalization"] == True:
            contour_u = network_output_u_axis.contourf(
                ...
                x_mesh_grid,
                (
                    network_output_u_over_training[frame]
        ...
        network_output_u_axis.set_title("Network Output for u at " + str(step) + "-th iteration")
    ...
    def update_network_output_h(frame, network_output_h_axis):
        ...
        if Hyper_Parameter_Dictionary["non_dimensionalization"] == True:
            contour_h = network_output_h_axis.contourf(
                time_mesh_grid,
                x_mesh_grid,
                (network_output_h_over_training[frame] * Hyper_Parameter_Dictionary["vertical_length_scale"]),
                cmap="coolwarm",
            ...
            contour_h = network_output_h_axis.contourf(
                time_mesh_grid,
                x_mesh_grid,
                network_output_h_over_training[frame],
                cmap="gist_rainbow",
                levels=np.arange(-1, 1, 0.01),
            )
        ...
        network_output_h_axis.set_title("Network Output for h at " + str(step) + "-th iteration")
    ...
    def update_symbolic_function_u(frame, symbolic_function_u_axis):
        ...
        contour_u = symbolic_function_u_axis.contourf(
            time_mesh_grid,
            x_mesh_grid,
            np.log10(abs(symbolic_function_u_over_training[frame])),
            cmap="coolwarm",
            levels=np.arange(-5, -3, 0.001),
        )
        ...
        symbolic_function_u_axis.set_title("PDE Loss for u at " + str(step) + "-th iteration")
    ...
    def update_symbolic_function_h(frame, symbolic_function_h_axis):
        ...
        contour_h = symbolic_function_h_axis.contourf(
            time_mesh_grid,
            x_mesh_grid,
            np.log10(abs(symbolic_function_h_over_training[frame])),
            cmap="coolwarm",
            levels=np.arange(-5, -3, 0.001),
        )
        ...
        symbolic_function_h_axis.set_title("PDE Loss for h at " + str(step) + "-th iteration")

    def update_MSE_terms(frame, learning_curve_axis):
        step = frame * Hyper_Parameter_Dictionary["training_steps"] / 1000.0
        learning_curve_axis.cla()
        learning_curve_axis.semilogy(training_steps, total_MSE_over_training, label=r"$MSE$")
        learning_curve_axis.semilogy(training_steps, MSE_boundary_conditions_over_training, label=r"$MSE_{BC}$")
        learning_curve_axis.semilogy(training_steps, MSE_initial_conditions_over_training, label=r"$MSE_{IC}$")
        learning_curve_axis.semilogy(training_steps, MSE_symbolic_functions_over_training, label=r"$MSE_{PDE}$")
        learning_curve_axis.semilogy(training_steps, Relative_L2_Error_h_over_training, label=r"$MSE_{SOL,h}$")
        learning_curve_axis.semilogy(training_steps, Relative_L2_Error_u_over_training, label=r"$MSE_{SOL,u}$")
        learning_curve_axis.axvline(x=step)
        learning_curve_axis.set_title(
            "Loss Functions at the "
            + str(step)
            + "-th of "
            + str(Hyper_Parameter_Dictionary["training_steps"])
            + " Iterations"
        )
        learning_curve_axis.set_xlabel("number of iterations")
        learning_curve_axis.set_ylabel(r"$MSE$ = $MES_{PDE}$ + $MSE_{BC}$ + $MSE_{IC}$")
    ...
    training_animation.save("PDE_Losses.gif", writer="Pillow", fps=int(frames_per_second))
